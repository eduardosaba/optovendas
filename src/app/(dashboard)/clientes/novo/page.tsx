...
import ConsultorioLogoBadge from "@/components/shared/OticaLogoBadge";
...
export default function CadastroClientePage() {
  ...
  const clienteIdEditar = searchParams.get("pacienteId") ?? searchParams.get("id");
  ...
  useEffect(() => {
    async function loadPrefill() {
      ...
      if (nomeFromQuery && !clienteIdEditar) {
        setPrefill((prev) => ({ ...prev, nome: nomeFromQuery }));
      }

      if (!clienteIdEditar) return;
      ...
      try {
        ...
        toast.error("Não foi possível carregar os dados do cliente para edição.");
      } finally {
    ...
    void loadPrefill();
  }, [clienteIdEditar, searchParams, toast]);
  ...
  async function adicionarClienteNaFila(clienteId: string) {
    ...
    if (!agenda?.agendaId) {
      ...
      const addRes = await supabase.from("agenda_pacientes").insert([
        { agenda_id: agenda.agendaId, paciente_id: clienteId },
      ]);
  ...
  async function responderPromptFila(addToQueue: boolean) {
    const clienteId = queueClienteId;
    setQueuePromptOpen(false);

    if (!addToQueue || !clienteId) {
      setQueueClienteId(null);
      return;
    }

    setAddingToQueue(true);
    try {
      await adicionarClienteNaFila(clienteId as string);
    } finally {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      let novoCliente: { id: string } | null = null;
      ...
      if (clienteIdEditar) {
        const updateRes = await supabase
          ...
          .select("id")
          .single();
        novoCliente = (updateRes.data as { id: string } | null) ?? null;
        ...
        const insertRes = await supabase
          ...
          .select("id")
          .single();
        novoCliente = (insertRes.data as { id: string } | null) ?? null;
      ...
      const clienteFinalId = (novoCliente as { id: string } | null)?.id ?? clienteIdEditar;

      if (clienteFinalId && (fotoFile || removerFoto)) {
        let publicUrl: string | null = null;

        if (fotoFile) {
          publicUrl = await uploadFotoPaciente(ctx.clinicaId, clienteFinalId, fotoFile);
        }
      ...
      toast.success(clienteIdEditar ? "Cadastro do cliente atualizado com sucesso!" : "Cliente cadastrado com sucesso!");
      ...
      if (clienteFinalId) {
        if (clienteIdEditar) {
          ...
          setQueueClienteId(clienteFinalId);
    ...
    <div className="mx-auto max-w-5xl p-6 md:p-10 space-y-8 pb-20">
      ...
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-blue-600 font-black text-xs uppercase tracking-widest">Clientes</p>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">
              {clienteIdEditar ? "Completar Cadastro" : "Novo Cliente"}
      ...
      {loadingPrefill && <p className="text-sm font-bold text-slate-400">Carregando dados do cliente...</p>}
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <section className="bg-white p-8 md:p-10 rounded-[32px] shadow-sm border border-slate-50 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputGroup id="nome" label="Nome Completo" placeholder="Nome completo do cliente" required defaultValue={prefill.nome} />
            <InputGroup id="apelido" label="Apelido / Como gosta de ser chamado" placeholder="Ex: Sr. João" defaultValue={prefill.apelido} />

            <div className="flex flex-col gap-2">
              <label htmlFor="cpf" className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">CPF</label>
              <PatternFormat
                id="cpf"
                name="cpf"
                format="###.###.###-##"
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="celular" className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Celular (WhatsApp)</label>
              <PatternFormat
                id="celular"
                name="celular"
                format="(##) #####-####"
        ...
        <section className="bg-white p-8 md:p-10 rounded-[32px] shadow-sm border border-slate-50 space-y-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-xl font-black text-slate-800 tracking-tight">Foto do Cliente</h2>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            <div className="h-28 w-28 rounded-[24px] border border-slate-200 bg-slate-50 overflow-hidden flex items-center justify-center">
              {fotoUrl ? (
                <img src={fotoUrl} alt="Foto do cliente" className="h-full w-full object-cover" />
              ) : (
        ...
        <div className="pt-4">
          ...
          >
            <Save size={24} />
            {loading ? "Salvando informações..." : clienteIdEditar ? "Atualizar Cadastro" : "Confirmar Cadastro"}
          </button>
        </div>
      </form>
      ...
      {queuePromptOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-md rounded-[28px] border border-slate-100 bg-white p-6 shadow-2xl">
            ...
            <h3 className="mt-2 text-xl font-black text-slate-900">Adicionar cliente na fila agora?</h3>
