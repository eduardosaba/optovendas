...
function ClienteCard({ cliente }: { cliente: any }) {
  // Função para limpar o telefone para o link do zap
  const zapLink = (tel: string) => {
    const num = (tel || "").replace(/\D/g, "");
    return `https://wa.me/55${num}`;
  };

  return (
    <div className="bg-white p-8 rounded-[40px] border border-slate-50 shadow-sm hover:shadow-2xl transition-all duration-500 group relative overflow-hidden">
      {/* Badge de Localidade */}
      ...
      <div className="mt-6 flex items-center gap-2 text-slate-400">
        ...
        <span className="text-xs font-bold text-slate-500">{cliente.cidade_atendimento || "Feira de Santana"}</span>
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-slate-50 flex items-center justify-between">
        <div className="flex gap-2">
          {/* WhatsApp */}
          <a 
            href={cliente.celular ? zapLink(cliente.celular) : "#"} 
            target="_blank"
            rel="noreferrer"
          ...
          {/* Editar Cadastro */}
          <Link
            href={`/clientes/novo?id=${cliente.id}`}
      ...
      {/* Botão de Ver Ficha Estilo "Acesso Rápido" */}
      <Link
