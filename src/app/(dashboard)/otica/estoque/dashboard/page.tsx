...
export default function DashboardEstoquePage() {
  ...
  async function carregar() {
    ...
    try {
      ...
      setResumo({
        ...
        valorCustoTotal: Number(
          resumoJson.valorCustoTotal || resumoJson.valorEstoque || 0,
        ),
        valorVendaTotal: Number(resumoJson.valorVendaTotal || 0),
      });

      setTopGrifes(
        ((topRes.data as any[]) || []).map((item) => ({
  ...
  const maxVendas = useMemo(
    () => Math.max(...topGrifes.map((g) => g.vendas), 1),
    [topGrifes],
  );
    ...
    <div className="mx-auto max-w-7xl p-6 md:p-10 space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="flex items-center gap-4">
          <Link
            href="/otica/estoque"
            className="p-3 bg-white rounded-2xl shadow-sm text-slate-400 hover:text-cyan-600 transition-all border border-slate-50"
          >
            <ArrowLeft size={20} />
          </Link>
          <div>
            <p className="text-cyan-600 font-black text-xs uppercase tracking-widest">
              Analítico
            </p>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">
              Kardex de Ótica<span className="text-cyan-600">.</span>
            </h1>
          </div>
        </div>
        ...
        <button
          onClick={carregar}
          className="p-4 bg-white rounded-2xl text-slate-400 hover:text-cyan-600 shadow-sm border border-slate-50 transition-all"
        >
      ...
      <DashboardGrid cols={4} gap="gap-6">
        ...
        <MetricCard
          ...
          value={resumo.totalPecas}
          icon={<TrendingUp size={20} />}
        />

        <MetricCard
          label="Custo Imobilizado"
          value={resumo.valorCustoTotal.toLocaleString("pt-BR", {
            style: "currency",
            currency: "BRL",
          })}
        ...
        <MetricCard
          ...
          value={resumo.valorVendaTotal.toLocaleString("pt-BR", {
            style: "currency",
            currency: "BRL",
          })}
        ...
        <div className="lg:col-span-3 bg-gradient-to-r from-cyan-600 to-cyan-500 p-8 rounded-[40px] text-white shadow-xl shadow-cyan-100 flex flex-col md:flex-row justify-between items-center group">
          <div>
            ...
            <h2 className="text-4xl font-black tracking-tighter mt-1">
              {(resumo.valorVendaTotal - resumo.valorCustoTotal).toLocaleString(
                "pt-BR",
          ...
          <div className="mt-4 md:mt-0 bg-white/10 p-4 rounded-3xl backdrop-blur-md border border-white/20">
            ...
            <p className="text-2xl font-black">
              {resumo.valorCustoTotal > 0
                ? (
                    ((resumo.valorVendaTotal - resumo.valorCustoTotal) /
                      resumo.valorCustoTotal) *
                    100
                  ).toFixed(0)
                : 0}
              %
          ...
          <div className="space-y-3">
            {baixoEstoque.length === 0 ? (
              ...
              baixoEstoque.map((item) => (
                <div
                  key={item.id}
                  className="group flex items-center justify-between p-4 bg-rose-50/50 rounded-2xl border border-rose-100/50 hover:bg-rose-50 transition-colors"
                >
                  <div>
        ...
        <section className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-50 space-y-8 lg:col-span-2">
          <div className="flex items-center justify-between border-b border-slate-50 pb-4">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">
                Performance por Grife
              </h3>
            </div>
            <span className="text-[10px] font-black text-cyan-600 bg-cyan-50 px-3 py-1 rounded-full uppercase">
              Últimos 30 Dias
            </span>
          </div>
          ...
          <div className="space-y-6">
            {topGrifes.length === 0 ? (
              <p className="text-center py-20 text-slate-300 font-bold italic text-sm tracking-tighter">
                Aguardando dados de vendas vinculadas...
              </p>
            ) : (
              topGrifes.map((grife) => {
                const pct = Math.max(
                  5,
                  Math.round((grife.vendas / maxVendas) * 100),
                );
                return (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right flex items-center gap-4">
                        ...
                        <span className="text-sm font-black text-slate-900">
                          R$ {grife.faturamento.toLocaleString("pt-BR")}
                        </span>
                      </div>
                    </div>
...
function MetricCard({
  ...
  return (
    <div
      className={`p-8 rounded-[40px] shadow-sm border transition-all hover:shadow-xl ${highlight ? "bg-slate-900 border-slate-800 text-white" : "bg-white border-slate-50 text-slate-900"}`}
    >
      <div
        className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 shadow-inner ${highlight ? "bg-white/10 text-cyan-400" : `bg-slate-50 ${colorClass}`}`}
      >
        {icon}
      </div>
      <p
        className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1 ${highlight ? "text-slate-500" : "text-slate-400"}`}
      >
        {label}
      </p>
