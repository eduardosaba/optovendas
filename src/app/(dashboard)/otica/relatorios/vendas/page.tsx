...
export default function RelatorioVendasPage() {
  ...
  async function handleCancelarVenda(id: string) {
    const confirmacao = window.confirm(
      "Tem certeza que deseja cancelar esta venda? O registro permanecerá no sistema com status 'cancelado'.",
    );
  ...
  const totalFiltrado = useMemo(() => {
    return vendasFiltradas
      ...
      .reduce(
        (acc, v) => acc + (Number(v.valor_final ?? v.valor_total) || 0),
        0,
      );
  }, [vendasFiltradas]);
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">
              Relatório de Vendas<span className="text-cyan-600">.</span>
            </h1>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-white border border-slate-200 text-slate-600 rounded-2xl font-bold text-xs uppercase hover:bg-slate-50 transition-all shadow-sm"
          >
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
        ...
        <div className="flex items-center gap-2 bg-slate-50 px-4 rounded-xl md:col-span-2">
          ...
          <input
            ...
            className="bg-transparent border-none py-3 font-bold text-slate-600 text-xs outline-none w-full"
          />
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                ...
                vendasFiltradas.map((v) => (
                  <tr
                    key={v.id}
                    className={`group hover:bg-slate-50/80 transition-all ${v.status_financeiro === "cancelado" ? "opacity-60 bg-slate-50/30" : ""}`}
                  >
                    ...
                    <td className="px-8 py-6">
                      <p
                        className={`text-sm font-black ${v.status_financeiro === "cancelado" ? "text-slate-400 line-through" : "text-slate-900"}`}
                      >
                        {Number(v.valor_final ?? v.valor_total).toLocaleString(
                          "pt-BR",
