...
export default function FechamentoCaixaPage() {
  ...
  const [periodo, setPeriodo] = useState<"dia" | "mes" | "semestre" | "ano">(
    "dia",
  );
  ...
  async function carregarDadosFechamento() {
    ...
    try {
      ...
      const fluxoQ = await supabase
        .from("fluxo_caixa")
        .select(
          "id,valor, tipo, localidade, referencia_id, origem, metodo_pagamento, descricao, data_movimento",
        )
      ...
      try {
        let vr: any = await supabase
          .from("vendas")
          .select(
            "id,valor_final,data_venda,criado_em,localidade,localidade_venda,metodo_pagamento,tipo_fechamento,pacientes(nome_completo),ordens_servico(numero_os)",
          )
          .eq("clinica_id", ctx.clinicaId)
        ...
        if (
          ...
          /data_venda|column .* does not exist/i.test(
            String(vr.error.message || vr.error),
          )
        ) {
          vr = await supabase
            .from("vendas")
            .select(
              "id,valor_final,criado_em,localidade,localidade_venda,metodo_pagamento,tipo_fechamento,pacientes(nome_completo),ordens_servico(numero_os)",
            )
            .eq("clinica_id", ctx.clinicaId)
            .gte("criado_em", deStr)
          ...
          if (
            ...
            vr = await supabase
              .from("vendas")
              .select(
                "id,valor_final,created_at,localidade,localidade_venda,metodo_pagamento,tipo_fechamento,pacientes(nome_completo),ordens_servico(numero_os)",
              )
              .eq("clinica_id", ctx.clinicaId)
      ...
      const fluxoRefs = new Set(
        (fluxoData || []).map((f: any) => String(f.referencia_id)),
      );
      const vendasAsMov = (vendasIntervalo || [])
        .map((v: any) => {
          const paciente =
            v.pacientes && v.pacientes[0] ? v.pacientes[0].nome_completo : null;
          const os =
            (v.ordens_servico && v.ordens_servico[0]
  ...
  const resumo = useMemo(() => {
    return movimentacoes.reduce(
      (acc, curr) => {
        ...
        const metodoRaw = (curr.metodo_pagamento || "")
          .toString()
          .toLowerCase();
        ...
        const metodo =
          ...
          (desc.includes("dinheiro")
            ...
            : desc.includes("pix")
              ? "pix"
              : desc.includes("cartao") || desc.includes("cartão")
                ? "cartao"
                : "");
  ...
  async function exportExcel() {
    try {
      exportToCsv(
        ...
        `fechamento_${new Date().toISOString().slice(0, 10)}.csv`,
      );
    } catch (e) {
      console.error(e);
        ...
        <Document>
          <Page style={pdfStyles.page}>
            ...
            <Text style={pdfStyles.title}>
              Fechamento de Rota - {new Date().toLocaleDateString()}
            </Text>
  ...
  async function handleFecharDia() {
    ...
    try {
      ...
      const { error } = await supabase
        .from("fechamentos_rota")
        .insert([payload]);
      if (error) {
        toast.error(
          "Falha ao registrar fechamento. Verifique a migration no banco.",
        );
      } else {
    ...
    <div className="mx-auto max-w-6xl p-6 md:p-10 space-y-8 animate-in fade-in duration-700 pb-20">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">
              Fechamento de Rota<span className="text-cyan-600">.</span>
            </h1>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Link
            href="/otica/financeiro/conciliacao"
            className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 border border-blue-200 px-4 py-3 rounded-2xl font-black text-[11px] uppercase tracking-wider hover:bg-blue-100 transition-all"
          >
      ...
      <DashboardGrid cols={4} gap="gap-6">
        ...
        <div className="bg-slate-900 p-6 rounded-[32px] shadow-xl text-white">
          ...
          <div className="mt-4 flex items-center gap-2 text-[10px] font-bold text-slate-400">
            ...
            {movimentacoes.length} Recebimentos
          </div>
        </div>
      </DashboardGrid>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50">
                  <th className="px-8 py-4 text-[9px] font-black uppercase text-slate-400">
                    Descrição
                  </th>
                  <th className="px-8 py-4 text-[9px] font-black uppercase text-slate-400">
                    Cidade
                  </th>
                  <th className="px-8 py-4 text-[9px] font-black uppercase text-slate-400 text-right">
                    Valor
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  <tr>
                    <td
                      colSpan={3}
                      className="p-10 text-center animate-pulse text-slate-300 font-black"
                    >
                  ...
                  <tr>
                    ...
                    >
                      Nenhum recebimento registrado nesta data.
                    </td>
                  </tr>
                ) : (
        ...
        <aside className="space-y-6">
          <div className="bg-white p-8 rounded-[40px] border border-slate-50 shadow-sm">
            ...
            <p className="text-sm text-slate-500 leading-relaxed mb-6 font-medium">
              Confira se o valor em dinheiro vivo no bolso coincide com o total
              de <strong>{brl(resumo.dinheiro)}</strong>.
            </p>
            <button
              onClick={handleFecharDia}
              disabled={loading}
              className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-900 transition-all shadow-lg shadow-emerald-100 disabled:opacity-60"
            >
          ...
          <div className="bg-amber-50 p-8 rounded-[40px] border border-amber-100">
            <div className="flex gap-4">
              ...
              <div>
                <p className="text-[10px] font-black text-amber-800 uppercase mb-1">
                  Dica de Gestão
                </p>
                <p className="text-xs text-amber-700 font-medium leading-relaxed">
                  Ao final de cada rota, exporte este relatório. Ele serve como
                  o seu comprovante de prestação de contas.
                </p>
              </div>
            </div>
          </div>
...
function ResumoCard({ label, value, color, icon }: any) {
  ...
  return (
    <div className="bg-white p-6 rounded-[32px] border border-slate-50 shadow-sm">
      <div
        className={`w-10 h-10 rounded-xl ${colors[color]} flex items-center justify-center mb-4`}
      >
        {icon}
      </div>
