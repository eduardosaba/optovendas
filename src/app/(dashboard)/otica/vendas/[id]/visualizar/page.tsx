        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">
              {(() => {
                const osNumero = venda?.numero_os_manual || (venda as any)?.numeroOsManual || (venda as any)?.numero_os || os?.numero_os || (venda?.id ? String(venda.id).slice(0,8).toUpperCase() : 'S/N');
                return `OS #${osNumero}`;
              })()}
            </h1>
          </div>
        ...
        <aside className="space-y-6">
          <div className="bg-slate-900 p-8 rounded-[40px] shadow-2xl text-white">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center border-b border-white/10 pb-4">
                ...
                <span className="font-bold">{(Number(venda.valor_total)).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
              </div>
              ...
              <div className="flex justify-between items-center pt-2">
                ...
                <span className="text-2xl font-black text-emerald-400">
                  {(Number(venda.valor_final ?? venda.valor_total)).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                </span>
              </div>
            </div>
            ...
            <div className="mt-8 p-4 bg-white/5 rounded-2xl border border-white/10">
              ...
              <p className="text-[11px] text-slate-400 leading-relaxed font-medium">
                Venda registrada por {venda.vendedor || 'Sistema'} em {new Date(venda.data_venda || venda.criado_em).toLocaleDateString()}.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
             ...
             <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-xs font-bold text-slate-600">
                <Calendar size={14} className="text-slate-300" /> Criada às {new Date(venda.data_venda || venda.criado_em).toLocaleTimeString()}
              </div>
               <div className="flex items-center gap-2 text-xs font-bold text-slate-600">
