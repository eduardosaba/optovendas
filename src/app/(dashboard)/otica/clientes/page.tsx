  ...
  async function handleConfirmDelete() {
    ...
    try {
      if (deleteAlsoAnamnese) {
        const tablesToDelete = ['receitas_optometricas','paciente_arquivos','laudos_funcionais','consultorio_receitas','atendimentos_clinicos','anamnese','ordens_servico'];
      ...
      if (error) {
        const msg = typeof (error as any).code === "string" && ((error as any).code === "23503" || (error as any).message?.includes("violates"))
          ? 'Não é possível excluir o cliente: existem registros dependentes (ex: receitas, anamnese). Marque a opção para removê-los primeiro.'
    ...
    <div className="max-w-7xl mx-auto p-6 md:p-10 space-y-8 animate-in fade-in duration-700">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {clientesFiltrados.map((cliente) => (
            <article key={cliente.id} className="bg-white p-8 rounded-[40px] border border-slate-50 shadow-sm hover:shadow-2xl transition-all duration-500 group relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => { setDeletingId(cliente.id); setDeletingName(cliente.nome_completo); setConfirmOpen(true); }} className="p-3 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-sm">
                    <Trash size={18} />
                  </button>
      ...
      >
        <label className="inline-flex items-center gap-3">
          ...
          <span className="text-sm text-slate-600">Excluir também receitas e anamnese vinculadas</span>
        </label>
      </ConfirmDialog>
    </div>
  );
