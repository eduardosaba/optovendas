...
function getConferenciaScore(os: OSConferencia) {
  ...
  if (typeof os.od_dnp === "number" && typeof os.oe_dnp === "number")
    score += 35;
  const coOk = typeof os.co_od === "number" && typeof os.co_oe === "number";
  const alturaOk =
    typeof os.altura_vertical_od === "number" &&
    typeof os.altura_vertical_oe === "number";
...
function getConferenciaLabel(score: number) {
  if (score >= 85)
    return {
      label: "Conferencia OK",
      classes: "bg-emerald-100 text-emerald-700",
    };
  if (score >= 50)
    return { label: "Revisar", classes: "bg-amber-100 text-amber-700" };
...
export default function DashboardConferenciaMedidasPage() {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      try {
        ...
        toast.error(
          `Erro ao carregar dashboard de conferencia: ${err.message}`,
        );
      } finally {
        setLoading(false);
      }
  ...
  const resumo = useMemo(() => {
    ...
    const incompleto = items.filter(
      (os) => getConferenciaScore(os) < 50,
    ).length;
    ...
    <div className="mx-auto max-w-7xl space-y-8 p-6 pb-20 md:p-10">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="flex items-center gap-4">
          <Link
            href="/otica/medidas"
            className="rounded-2xl border border-slate-100 bg-white p-3 text-slate-400 shadow-sm transition-all hover:text-cyan-600"
          >
            <ArrowLeft size={20} />
          </Link>
          <div>
      ...
      <DashboardGrid cols={4} gap="gap-3">
        <ResumoCard
          icon={<Eye size={16} />}
          label="Ultimas fotos"
        ...
        <ResumoCard
          icon={<Clock3 size={16} />}
          label="Revisar"
          value={String(resumo.revisar)}
          tone="amber"
        />
        <ResumoCard
          icon={<CircleAlert size={16} />}
          label="Incompleto"
          value={String(resumo.incompleto)}
          tone="rose"
        />
      </DashboardGrid>
      ...
      {loading ? (
        ...
        <div className="rounded-[32px] border border-dashed border-slate-200 bg-slate-50 p-16 text-center">
          ...
          <p className="text-sm font-black uppercase tracking-wider text-slate-500">
            Nenhuma foto de pupilometro encontrada.
          </p>
        </div>
      ) : (
        <>
          ...
          <section className="bg-white p-4 rounded-[32px] border border-slate-100 shadow-sm flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="flex flex-wrap gap-2 flex-1">
              ...
              <BotaoFiltro
                ...
                label="Revisar"
                count={resumo.revisar}
                color="amber"
              />
              <BotaoFiltro
                ativo={filtroStatus === "incompleto"}
                onClick={() => setFiltroStatus("incompleto")}
                label="Críticos"
                count={resumo.incompleto}
                color="rose"
              />
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            {itensFiltrados.map((os) => {
              ...
              const vendedorNome = vendedorId
                ? vendedoresMap[vendedorId] || "Sem nome"
                : "Nao informado";
              const score = getConferenciaScore(os);
              ...
              return (
                <article
                  key={os.id}
                  className="group overflow-hidden rounded-[30px] border border-slate-100 bg-white shadow-sm hover:shadow-md transition-all"
                >
                  <div className="relative aspect-video w-full overflow-hidden bg-slate-100">
                    {os.pupilometro_foto_url ? (
                      <>
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                          <button
                            onClick={() =>
                              setFotoExpandida(os.pupilometro_foto_url!)
                            }
                    ...
                    <div
                      className={`absolute right-4 top-4 rounded-xl px-3 py-1.5 text-[10px] font-black uppercase shadow-sm ${status.classes}`}
                    >
                      {status.label}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 p-5">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] font-bold text-slate-600">
                      <span className="rounded-xl bg-slate-50 px-3 py-2">
                        DNP OD:{" "}
                        {typeof os.od_dnp === "number"
                          ? os.od_dnp.toFixed(1)
                          : "--"}
                      </span>
                      <span className="rounded-xl bg-slate-50 px-3 py-2">
                        DNP OE:{" "}
                        {typeof os.oe_dnp === "number"
                          ? os.oe_dnp.toFixed(1)
                          : "--"}
                      </span>
                      <span className="rounded-xl bg-slate-50 px-3 py-2">
                        CO OD:{" "}
                        {typeof os.co_od === "number"
                          ? os.co_od.toFixed(1)
                          : "--"}
                      </span>
                      <span className="rounded-xl bg-slate-50 px-3 py-2">
                        CO OE:{" "}
                        {typeof os.co_oe === "number"
                          ? os.co_oe.toFixed(1)
                          : "--"}
                      </span>
                    </div>
      ...
      {fotoExpandida && (
        ...
        >
          <img
            src={fotoExpandida}
            className="max-h-full max-w-full rounded-2xl shadow-2xl border-4 border-white/10"
            alt="Zoom Medição"
          />
          <p className="absolute bottom-6 text-white/50 font-black uppercase text-[10px] tracking-widest">
...
function BotaoFiltro({
  ...
  return (
    ...
    >
      {label}{" "}
      <span
        className={`px-2 py-0.5 rounded-lg text-[9px] ${ativo ? "bg-white/20" : "bg-black/5"}`}
      >
        {count}
      </span>
    </button>
