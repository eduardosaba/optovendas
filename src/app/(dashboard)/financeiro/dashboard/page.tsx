    ...
    async function carregar() {
      ...
      try {
        ...
        try {
          ...
          console.warn("installments read failed in dashboard, trying payments fallback:", e?.message || e);
          try {
            const { data: d2, error: err2 } = await supabase
              .from("payments")
              .select("id, valor_parcela, vencimento, status, pago_em, vendas(localidade_venda)")
              .eq("clinica_id", ctx.clinicaId)
              .order("vencimento", { ascending: true });

            if (err2) throw err2;
            setRows((d2 as InstallmentRow[]) ?? []);
          } catch (e2: any) {
