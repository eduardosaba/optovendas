    ...
    <div className="mx-auto max-w-7xl p-6 md:p-10 space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="flex items-center gap-4">
          <Link
            href="/admin"
            className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-2xl border border-slate-50 bg-white p-3 text-slate-400 shadow-sm transition-all hover:text-blue-600"
          >
            <ArrowLeft size={20} />
          </Link>
          <div>
            <p className="text-blue-600 font-black text-xs uppercase tracking-[0.3em] mb-1">
              Análise de Funil
            </p>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">
              Performance Global<span className="text-blue-600">.</span>
            </h1>
          </div>
        </div>
        ...
        <div className="flex bg-white p-1 rounded-2xl border border-slate-50 shadow-sm">
          <Link
            ...
            className="inline-flex min-h-11 items-center px-6 py-3 text-[10px] font-black uppercase text-slate-400 transition-all hover:text-blue-600"
          >
            Relatório Geral
          </Link>
          <div className="w-px h-4 bg-slate-100 self-center" />
          <span className="inline-flex min-h-11 items-center rounded-xl bg-blue-50 px-6 py-3 text-[10px] font-black uppercase text-blue-600">
            Conversão Ótica
          </span>
        </div>
      </header>
              ...
              <tbody className="divide-y divide-slate-50">
                {rows.length === 0 ? (
                  <tr>
                    <td
                      colSpan={5}
                      className="p-20 text-center text-slate-400 italic font-bold text-sm"
                    >
                      Nenhum dado de performance processado.
                    </td>
                  </tr>
                ) : (
                  rows.map((r, i) => (
                    ...
                    >
                      <td className="p-8">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-black text-slate-800 tracking-tight uppercase text-sm">
                              {r.cidade}
                            </p>
                            <p className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                              <Calendar size={10} />{" "}
                              {new Date(
                                `${r.data_atendimento}T00:00:00`,
                          ...
                          <div className="flex flex-col items-end">
                            ...
                            <div className="w-24 h-1.5 bg-slate-100 rounded-full mt-2 overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, Number(r.taxa_conversao || 0))}%`,
                                }}
                              />
                            </div>
                          </div>
...
function PerformanceKpi({ label, value, sub, icon, color }: any) {
  ...
  return (
    <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-50 relative overflow-hidden group">
      <div
        className={`w-12 h-12 ${colors[color]} rounded-2xl flex items-center justify-center mb-6 shadow-inner transition-transform group-hover:scale-110`}
      >
        {icon}
      </div>
