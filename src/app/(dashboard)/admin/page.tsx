...
import {
  Users, Globe, DollarSign, Zap, Shield,
  Clock, CheckCircle2, XCircle, Search,
  Settings, ArrowUpRight, Filter, MoreVertical,
  Calendar, AlertTriangle, Lock, Unlock
} from "lucide-react";
...
export default function TorreDeControleMaster() {
  ...
  async function atualizarClinica(id: string, updates: Partial<Clinica>) {
    const { error } = await supabase.from("clinicas").update(updates).eq("id", id);
    if (error) {
  ...
  async function prorrogarTrial(clinica: Clinica) {
    ...
    await atualizarClinica(clinica.id, { data_vencimento: novaData.toISOString() } as any);
  }
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
             ...
             <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-600">Nível: Super Administrador</p>
          </div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">Torre de Controle <span className="text-slate-400">Master</span></h1>
        </div>

        <div className="flex gap-3">
           <Link href="/admin/configuracoes" className="px-6 py-3 bg-white border border-slate-200 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm">Configurações SaaS</Link>
           <button onClick={carregarDadosMaster} className="p-3 bg-slate-900 text-white rounded-2xl hover:bg-cyan-600 transition-all shadow-lg"><Globe size={20}/></button>
        </div>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <MasterStatCard label="Clínicas Ativas" value={stats.clinicasTotais} icon={<Globe className="text-blue-600"/>} color="blue" />
        <MasterStatCard label="Novos Trials (14d)" value={stats.emTrial} icon={<Zap className="text-amber-600"/>} color="amber" />
        <MasterStatCard label="Pacientes na Rede" value={stats.pacientesBase} icon={<Users className="text-emerald-600"/>} color="emerald" />
        <div className="bg-slate-900 p-8 rounded-[40px] shadow-xl text-white">
           <p className="text-[10px] font-black uppercase text-slate-500 mb-1">Transacionado Global</p>
           <p className="text-2xl font-black text-emerald-400">R$ {stats.faturamentoEstimado.toLocaleString()}</p>
        </div>
      </div>
      ...
      <section className="bg-white p-6 rounded-[40px] border border-slate-50 shadow-sm space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
           ...
           <div className="flex gap-2">
              <button className="px-4 py-2 bg-slate-100 rounded-xl text-[10px] font-black uppercase text-slate-500">Todos Planos</button>
              <button className="px-4 py-2 bg-slate-100 rounded-xl text-[10px] font-black uppercase text-slate-500">Expira Hoje</button>
           </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400">Clínica / Cadastro</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400">Plano / Status</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400">Vencimento</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 text-right">Ações de Admin</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {clinicasFiltradas.map(c => {
                const diasRestantes = Math.ceil((new Date(c.data_vencimento).getTime() - Date.now()) / (1000 * 3600 * 24));
                const isExpirado = diasRestantes <= 0;

                return (
                  <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-5">
                       <p className="font-black text-slate-800 text-sm">{c.nome_fantasia}</p>
                       <p className="text-[10px] text-slate-400 uppercase font-bold">{c.cidade_sede || 'Local não informado'} • Criado em {new Date(c.criado_em).toLocaleDateString('pt-BR')}</p>
                    </td>
                    <td className="px-6 py-5">
                       <div className="flex flex-col gap-1">
                          ...
                          <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full w-fit ${c.status === 'ativo' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}>
                            {c.status}
                          </span>
                       </div>
                    </td>
                       ...
                       <div className="flex items-center gap-2">
                          ...
                          <div>
                             <p className={`text-xs font-black ${isExpirado ? 'text-rose-600' : 'text-slate-700'}`}>
                               {new Date(c.data_vencimento).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })}
                             </p>
                    ...
                    <td className="px-6 py-5 text-right">
                       <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                          ...
                          <button 
                            onClick={() => atualizarClinica(c.id, { status: c.status === 'ativo' ? 'suspenso' : 'ativo' })}
