...
export async function POST(req: NextRequest) {
  ...
  // Simple auth: require internal key
  const internalKey = req.headers.get('x-internal-key') || '';
  if (!internalKey || internalKey !== process.env.INTERNAL_API_KEY) return NextResponse.json({ error: 'unauthorized' }, { status: 403 });
  ...
  try {
    ...
    // Insert fluxo_caixa entry (valor líquido) and record valor_bruto + taxa
    const fluxo = {
    ...
    // Update venda: mark payment confirmed and update financeiro-related flags
    const updateObj: any = {
      status_financeiro: 'confirmado',
      status_pagamento: 'confirmado',
    };
    // If valor provided, update valor_entrada to reflect confirmed amount
    ...
    if (upd.error) console.warn('confirm-payment: failed updating venda', upd.error);
